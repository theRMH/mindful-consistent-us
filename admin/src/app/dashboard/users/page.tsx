...
export default async function UsersDirectoryPage({
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-extrabold text-gray-900 font-sans">Users Directory</h2>
        <p className="text-sm text-gray-500 mt-1">Search registered users, track progress stats, and review wellness logs.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 items-start">
        ...
        <div className={`bg-white shadow rounded-lg border border-gray-100 overflow-hidden lg:col-span-2`}>
          {profiles.length === 0 ? (
            <div className="p-20 text-center flex flex-col items-center">
              ...
              <p className="text-sm text-gray-400 mt-1">Try refining your search terms or view the entire list.</p>
            </div>
          ) : (
              ...
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Phone / Email</th>
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <div className="font-bold text-gray-900">{profile.fullName || 'No name provided'}</div>
                              <div className="text-xs text-gray-400">ID: {profile.id.substring(0, 8)}...</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-700 font-semibold">{profile.phone || 'No phone'}</div>
                          <div className="text-xs text-gray-400">{profile.email}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center space-x-1">
                            <span className="text-sm font-bold text-gray-900">{streak}</span>
                            {streak > 0 && <span className="text-amber-500 text-sm">🔥</span>}
                          </div>
                        </td>
        ...
        <div className="space-y-6">
          {selectedUser ? (
            <>
              ...
              <div className="bg-white shadow rounded-lg border border-gray-100 p-6 space-y-4">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Joined {new Date(selectedUser.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
                ...
                <div className="border-t border-gray-50 pt-4 space-y-2.5 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-800 font-bold">{selectedUser.email}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400 font-medium">Phone:</span>
                    <span className="text-gray-800 font-bold">{selectedUser.phone || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-800 font-bold">{selectedUser.userStats?.longestStreak ?? 0} days</span>
                  </div>
                </div>
              </div>
              ...
              {/* User Aggregates */}
              <div className="bg-white shadow rounded-lg border border-gray-100 p-6 space-y-4">
                <h4 className="font-extrabold text-gray-800 border-b border-gray-50 pb-2">Practice Aggregates</h4>
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div className="bg-blue-50/50 border border-blue-50 p-3 rounded-lg">
                    <div className="text-xs font-bold text-blue-500 uppercase">Minutes</div>
                    <div className="text-lg font-black text-blue-900 mt-1">
                      {Math.round((selectedUser.userStats?.totalWatchSeconds ?? 0) / 60)}
                    </div>
                  </div>
                  <div className="bg-amber-50/50 border border-amber-50 p-3 rounded-lg">
                    <div className="text-xs font-bold text-amber-500 uppercase">Steps</div>
                    <div className="text-lg font-black text-amber-900 mt-1">
                      {(selectedUser.userStats?.totalSteps ?? 0).toLocaleString()}
                    </div>
                  </div>
                  <div className="bg-emerald-50/50 border border-emerald-50 p-3 rounded-lg">
                    <div className="text-xs font-bold text-emerald-500 uppercase">Calories</div>
                    <div className="text-lg font-black text-emerald-900 mt-1">
                      {Number(selectedUser.userStats?.totalCalories ?? 0).toFixed(0)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Course Enrollments */}
              <div className="bg-white shadow rounded-lg border border-gray-100 p-6 space-y-4">
                <h4 className="font-extrabold text-gray-800 border-b border-gray-50 pb-2">Enrolled Programs</h4>
                {selectedUser.enrollments.length === 0 ? (
                  <p className="text-sm text-gray-400 italic">No active enrollments.</p>
                ) : (
                  <div className="space-y-3">
                    {selectedUser.enrollments.map((enr: any) => (
                      <div key={enr.id} className="p-3 bg-gray-50 rounded-lg border border-gray-100 text-sm">
                        <div className="font-bold text-gray-800">{enr.course.title}</div>
                        <div className="flex justify-between text-xs text-gray-500 mt-1.5 font-medium">
                          <span>Bought: {new Date(enr.purchaseDate).toLocaleDateString()}</span>
                          <span className="capitalize text-emerald-600 font-bold">{enr.paymentStatus}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Daily Progress logs */}
              <div className="bg-white shadow rounded-lg border border-gray-100 p-6 space-y-4">
                <h4 className="font-extrabold text-gray-800 border-b border-gray-50 pb-2">Daily Progress Log</h4>
                {dailyProgresses.length === 0 ? (
                  <p className="text-sm text-gray-400 italic">No workout logs registered yet.</p>
                ) : (
                  <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                    {dailyProgresses.map((log) => (
                      <div key={log.id} className="p-3 bg-gray-50 rounded-lg border border-gray-100 text-xs space-y-1">
                        <div className="flex justify-between items-center font-bold">
                          <span className="text-gray-800">{new Date(log.dayDate).toLocaleDateString()}</span>
                          {log.isComplete ? (
                        ...
                        <p className="text-gray-500 font-semibold">{log.courseDay.course.title} — Day {log.courseDay.dayNumber}</p>
                        <div className="grid grid-cols-3 text-gray-400 font-semibold pt-1 border-t border-gray-100/50 mt-1 text-[10px]">
                          ...
                          <span>🔥 {Number(log.caloriesBurnt).toFixed(1)} kcal</span>
                          <span>⏱️ {Math.round(log.totalWatchSeconds / 60)}m watch</span>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="bg-gray-50 border border-dashed border-gray-200 rounded-lg p-8 text-center text-sm text-gray-400 font-semibold">
              Select a user from the directory to display progress graphs, streak summaries, and practice logs.
            </div>
          )}
        </div>
      </div>
