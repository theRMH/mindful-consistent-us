...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const isMobileAppActive = pathname.startsWith('/dashboard/mobile-app');
  const [mobileAppOpen, setMobileAppOpen] = useState(isMobileAppActive);
  ...
  const navItems = [
    ...
    {
      name: 'Community',
      path: '/dashboard/community-moments',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const mobileAppChildren = [
    {
      name: 'Content',
      path: '/dashboard/mobile-app/content',
      icon: (
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          ...
          <div className="pt-2">
            <div className="px-2 pb-1">
              <span className="text-[10px] font-bold uppercase tracking-widest text-green-500/60 px-2">
                Mobile App
              </span>
            </div>
            <button
            ...
            >
              ...
              <svg
                className={`w-4 h-4 transition-transform duration-200 ${mobileAppOpen ? 'rotate-180' : ''}`}
            ...
            {mobileAppOpen && (
              <div className="mt-1 ml-4 space-y-1 border-l border-green-800/50 pl-3">
                {mobileAppChildren.map((child) => {
